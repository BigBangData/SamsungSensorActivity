...
x <- sub("tBody", "TimeBody", x); x <- sub("tGravity", "TimeGravity", x); x <- sub("fBody", "FreqBody", x)
x <- sub("BodyBody", "Body", x); x <- sub("-mean\\(\\)-X", "X.mean", x); x <- sub("-mean\\(\\)-Y", "Y.mean", x)
x <- sub("-mean\\(\\)-Z", "Z.mean", x); x <- sub("-std\\(\\)-X", "X.sd", x); x <- sub("-std\\(\\)-Y", "Y.sd", x)
x <- sub("-std\\(\\)-Z", "Z.sd", x); x <- sub("-meanFreq\\(\\)-X", "X.meanFreq", x)
x <- sub("-meanFreq\\(\\)-Y", "Y.meanFreq", x); x <- sub("-meanFreq\\(\\)-Z", "Z.meanFreq", x)
x <- sub("-mean\\(\\)", ".mean", x); x <- sub("-std\\(\\)", ".sd", x); x <- sub("-meanFreq\\(\\)", ".meanFreq", x)
names(data_subset) <- x
