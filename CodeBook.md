...
The dataset has 81 variables. The first two variables are a person-activity set of ID variables. The 79 remaining variables are average values, for each person-activity pair, of mean, standard deviation, and mean frequeny measurements selected from the original data. Calling the str() function on the tidy set provides a look at the variable names, classes, levels, and first few values:
...
From the original 'features_info.txt' file: 
